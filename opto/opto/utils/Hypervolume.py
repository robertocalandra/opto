...
class HV:
    ...
    Based on Simon Wessing's reimplementation of the code by Fonseca et al. (Variant 3, Version 1.2) in pure Python
...
def HyperVolume(objectives, referencePoint, verbosity=0, indent=0, computePF=True):
    """
    
    :param objectives: [N_OBJECTIVES, N_POINTS]
    :param referencePoint: [N_OBJECTIVES]
    :param verbosity: 
    :param indent: 
    :param computePF: if True pre-process the parameters by computing their PF and using it to compute the Hypervolume.
    This pre-process step often reduce the computational time.
    :return: 
    """

    assert objectives.shape[0] == referencePoint.size, 'Invalid size of objectives/referencePoint'

    if computePF:
        # Compute PF to reduce number of points
        objectives = paretoFront(objectives)

    objectives = objectives.T
    hv = HV(referencePoint)
    out = hv.compute(objectives)
